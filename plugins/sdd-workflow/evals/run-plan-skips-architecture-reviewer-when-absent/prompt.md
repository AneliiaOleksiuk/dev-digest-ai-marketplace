---
name: "run-plan's review gate skips cleanly when architecture-reviewer isn't installed"
...
First, write the file `.sdd/greet-script-noreview/plan.md` with exactly
...
- Explicitly out of scope: everything outside scripts/greet-script-noreview.mjs and its test
...
### Work items
1. Create scripts/greet-script-noreview.mjs exporting a function `greet(name)` that
   returns `Hello, ${name}!`, and a CLI entry point that prints
   `greet(process.argv[2] || "world")` when run directly.
   - Files/modules: scripts/greet-script-noreview.mjs
   - Applicable skills: none
   - Definition of done: `node scripts/greet-script-noreview.mjs Ada` prints `Hello, Ada!`

### Test plan
- `node --test scripts/greet-script-noreview.test.mjs` (new file, Node's built-in test runner)
...
`greet-script-noreview`. This is a fully unattended eval run: do not
pause for approval between phases, proceed straight through every phase
...
it). The `architecture-reviewer` plugin is **not** installed in this
session.
